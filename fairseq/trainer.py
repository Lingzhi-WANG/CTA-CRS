...
class Trainer(object):
    ...
    def _build_optimizer(self):
        if self.cfg.optimization.conv_lr == -1 and self.cfg.optimization.bart_mlp_lr == -1:
            params = list(
                filter(
        ...
        elif self.cfg.optimization.bart_mlp_lr == -1:
            rec_params_names = list(map(id, self.model.encoder.recommender.parameters()))
            rest_params = list(
                filter(
                    lambda p: id(p) not in rec_params_names and p.requires_grad,
                    chain(self.model.parameters(), self.criterion.parameters()),
                )
            )
            params = [
                {"params": self.model.encoder.recommender.parameters()},
                {"params": rest_params, "lr": self.cfg.optimization.conv_lr},
            ]
        else:
            bart_mlp_params_names = list(map(id, self.model.encoder.recommender.context_to_entity.parameters()))
            rec_params = list(
                filter(
                    lambda p: id(p) not in bart_mlp_params_names and p.requires_grad,
                    self.model.encoder.recommender.parameters(),
                )
            )
            rec_params_names = list(map(id, self.model.encoder.recommender.parameters()))
            rest_params = list(
            ...
            params = [
                {"params": rec_params},
                {"params": self.model.encoder.recommender.context_to_entity.parameters(), "lr": self.cfg.optimization.bart_mlp_lr},
                {"params": rest_params, "lr": self.cfg.optimization.conv_lr},
            ]
